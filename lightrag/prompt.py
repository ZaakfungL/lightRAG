...
Your goal is to provide a structured answer to the user's query. The answer MUST be generated by integrating relevant facts from the Document Chunks found in the **Context**.
The output MUST follow the exact format shown below with each field on a separate line.
...
Reference ID: [mapped_reference_ids]
...
1. **Answer**: Provide the shortest possible keywords or key phrases that directly answer the question.
...
2. **Answer Value**:
   - CRITICAL: This field MUST ALWAYS contain a normalized value based on the Answer field. NEVER leave as "is_blank" when you have provided an Answer.
   - For numeric answers: Extract and calculate the actual numeric value ALWAYS in base units.
     - **Percentage answers**: Convert "12-34%" → "[12,34]", "56.7%" → "56.7", "89.0%" → "89.0"
     - **Range answers**: Use list format "[12,34]" or single number "56"
     - **Decimal numbers**: "12.3", "0.45", "6.7"
     - **Integer numbers**: "123", "45", "6"
     - **Magnitude conversions**: "1.234 million" → "1234000", "2.5 billion" → "2500000000"
     - Extract just the pure number (e.g., "1234000", "56.7", "[12,34]").
   - For noun answers: Use the same noun as in the answer field.
   - For TRUE/FALSE answers: When the answer is "TRUE" or "FALSE", the answer_value should be "1" for TRUE or "0" for FALSE.
   - If truly no answer is possible (and Answer field is "Unable to answer"): use "is_blank".
   - **Examples**:
     * Answer: "12-34%" → Answer Value: "[12,34]"
     * Answer: "56.7%" → Answer Value: "56.7"
     * Answer: "89.0%" → Answer Value: "89.0"
     * Answer: "123 to 456 units" → Answer Value: "[123,456]"
     * Answer: "TRUE" → Answer Value: "1"
     * Answer: "Some concept" → Answer Value: "Some concept"

3. **Answer Unit**:
   - Provide unit of measurement (e.g., "tCO2e", "MWh", "GB", "percent", "kg/GPU").
   - If a specific answer unit is requested in the Additional Instructions below, use that exact unit format.
...
2. Document ID Mapping: ALWAYS map document file names to reference IDs using the provided mapping table in the Reference ID field.
3. Unanswerable Questions: If you cannot answer with confidence based on the provided documents, you MUST use "is_blank" for Answer Value, Reference ID, Supporting Materials, and Explanation.
4. **CRITICAL: Numeric Value Processing**
   - ALWAYS convert magnitude expressions to actual numbers in Answer Value field
   - Examples: "1.2 billion" → Answer Value: "1200000000"
   - Examples: "3.5 thousand" → Answer Value: "3500"
   - Examples: "12-34%" → Answer Value: "[12,34]"
   - The Answer field can contain the original expression, but Answer Value MUST be the calculated number
